<template>
  <el-card class="box-card" shadow="never">
    <div style="display: flex;justify-content: space-between;">
      <h1 style="cursor: pointer;" @click="toContest(contest.id)">
        最新比赛：<span style="color: #659fdc">{{ contest.contest_title }}</span>
      </h1>
...
export default {
  ...
  //跑马灯高度自适应
  mounted() {
    getRequest("/get_last_contest").then(res => {
      if (res.data){
        console.log(res);
        let data = res.data.contest;
        this.contest = data;
        let nowTime = new Date();
        let regStartTime = new Date(data.reg_start_time);
        let regEndTime = new Date(data.reg_end_time);
        let startTime = new Date(data.start_time);
        let endTime = new Date(data.end_time);
        this.contest.id = this.contest._id["$oid"]
